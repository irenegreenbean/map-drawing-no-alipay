...
const sort_trial = {
    ...
    prompt: "<p>每一个方块是一段不同的音频。您可以按一下音频来播放。请您把每一段音频都听一遍（至少一遍）然后将他们根据音频的相似度分组。</p><p>分组时，您需要按下某个方块然后将它继续按下，然后您就可以把那个方块拉到中间的大长方形了。</p><p>您分组时，请把您觉得听起来一样或相似的音频都放在一起。您想把每个音频听几遍，就听几遍。每一组不一定得是同样的大小，也不限制您总共分多少组。</p><p>您把音频在大长方形里全部分组完会在屏幕的下部看到继续的按钮。分组后，继续按钮将显示 ’Continue‘ 这个英语字，您按就可以继续了。</p><p>屏幕上的其他英语字您不用去管。</p>",
    side_width: 150
    //choices: ['Continue'],
};
